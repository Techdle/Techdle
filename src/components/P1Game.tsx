...
export function P1Game() {
  const { puzzle, state, isLoaded, submitGuess, MAX_GUESSES, incorrectCount, isSubmitting } = useP1OutageGame();
  ...
  return (
    <div className="w-full max-w-4xl mx-auto py-8 px-4 flex flex-col gap-8">
      ...
      {!isGameOver ? (
        <GuessInput onSubmit={submitGuess} disabled={isSubmitting} shakeKey={incorrectCount} targets={[]} />
      ) : (
